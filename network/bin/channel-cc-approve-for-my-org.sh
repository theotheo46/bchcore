...
hlf/bin/peer lifecycle chaincode approveformyorg --peerAddresses=${CORE_PEER_ADDRESS} --tlsRootCertFiles=${CORE_PEER_TLS_ROOTCERT_FILE} --tls true --cafile $ORDERER_CA --channelID cnft --name cnft --version 1.0 --package-id ${PACKAGE_ID} --sequence 1
