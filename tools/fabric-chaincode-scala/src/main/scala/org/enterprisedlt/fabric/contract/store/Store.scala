...
class Store(
    ...
    def listKeys[T <: AnyRef](clz: Class[T], key: Key): Iterable[Key] = {
    ...
    private[this] def convertKeyValue[T](v: QueryResultsIterator[ledger.KeyValue], clz: Class[T]): Iterable[KeyValue[T]] =
        v.asScala.map(kv => KeyValue(unwrapKeyToString(clz, kv.getKey), codec.decode[T](kv.getValue, clz)))
    ...
    private[this] def unwrapKeyToString(clz: Class[_], key: String): String =
        CompositeKey.parseCompositeKey(key).getAttributes.asScala.mkString(CompositeKey.NAMESPACE)

    private[this] def unwrapKey(clz: Class[_], key: String): Key =
        Key(CompositeKey.parseCompositeKey(key).getAttributes.asScala: _*)
